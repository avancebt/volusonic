        ...
        if (curUri.startsWith('volusonic')) {
          if (curUri == 'volusonic') {
            ...
            if (uriParts.length == 2) {
              ...
              if (uriParts[2].length <= 2) {
                response = self.listArtistsForLetter(uriParts, curUri);
              } else {
            ...
            if (curUri == 'volusonic/favorites') {
              ...
              response = self.listFavorites(uriParts, 'volusonic/favorites');
            }
          }
        }
...
ControllerVolusonic.prototype.showArtist = function(uriParts, curUri) {
  ...
  var artistPromise = self._artist(id);
  var infoPromise = self.config.get('metas') ? self._artistInfos(id).fail(function() { return {}; }) : libQ.resolve({});

  libQ.all([artistPromise, infoPromise])
    .then(function(results) {
      var artist = results[0];
      var infos = results[1] || {};

      if (!artist) {
        defer.reject(new Error('Artist not found'));
        return;
      }
      ...
      } else if (artist.artistImageUrl) {
        info.albumart = artist.artistImageUrl;
      } else {
        info.albumart = '/albumart?sourceicon=music_service/volusonic/subso.png';
      }

      var meta = "Magic Playlist";
      if (infos.biography && self.config.get('metas')) {
      ...
      if (artist[container] !== undefined) {
        ...
        nav.navigation.lists.push(albumsList);
      }

      if (infos.similarArtist !== undefined) {
        var arts = [];
        var similarArtists = Array.isArray(infos.similarArtist) ? infos.similarArtist : [infos.similarArtist];
        similarArtists.forEach(function(similar) {
          if (similar && similar.name && similar.id) {
        ...
        if (arts.length > 0) {
          ...
          nav.navigation.lists.push(similarsList);
        }
      }

      self._topSongs(encodeURIComponent(artist.name), '10')
        .then(function(top) {
          if (top && top.song) {
            var sgs = [];
            var songsArray = Array.isArray(top.song) ? top.song : [top.song];
            ...
            nav.navigation.lists.splice(1, 0, topSongsList);
          }
...
ControllerVolusonic.prototype._formatArtist = function(artist, curUri) {
  ...
  if (artist.coverArt) {
    albumart = self.config.get('server') + '/rest/getCoverArt.view?id=' + artist.coverArt + '&size=' + self.getSetting('artSize') + '&' + self.config.get('auth');
  } else if (artist.artistImageUrl) {
...
ControllerVolusonic.prototype.listArtists = function(uriParts, curUri) {
  ...
  self.backend.get(getArtists, 'Artists', '')
    .then(function(result) {
      var items = [];
      var indexes = (result && result['subsonic-response'] && result['subsonic-response'][container] && result['subsonic-response'][container]['index']) || [];
      indexes.forEach(function(index) {
        var letter = index.name;
        var count = index.artist ? index.artist.length : 0;
        if (count > 0) {
          items.push({
            service: 'volusonic',
            type: 'folder',
      ...
      defer.resolve(self._formatNav(
        self.commandRouter.getI18nString('ARTISTS'),
        'folder',
        'fa fa-microphone',
        ['list', 'grid'],
        items,
...
ControllerVolusonic.prototype.listArtistsForLetter = function(uriParts, curUri) {
  var self = this;
  var defer = libQ.defer();
  ...
  self.backend.get(getArtists, 'Artists', '')
    .then(function(result) {
      var items = [];
      var indexes = (result && result['subsonic-response'] && result['subsonic-response'][container] && result['subsonic-response'][container]['index']) || [];
      var targetIndex = indexes.find(function(idx) {
        return idx.name === letter;
      });

      if (targetIndex && targetIndex.artist) {
        targetIndex.artist.forEach(function(artist) {
...
ControllerVolusonic.prototype.search = function(query) {
  ...
  var id = encodeURIComponent(queryVal);
  var searchCount = Math.min(parseInt(self.getSetting('listsSize')) || 100, 100);
  var params = "query=" + encodeURIComponent(queryVal) + "&artistCount=" + searchCount + "&albumCount=" + searchCount + "&songCount=" + searchCount;
